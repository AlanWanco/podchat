...
- 导入本地音频和 ASS 字幕文件
- 支持导入 `ASS / SRT / LRC` 字幕文件（SRT/LRC 默认使用首个普通说话人）
- 导入 ASS 时可检测 Name / Style，并可选择把 ASS 样式映射到说话人气泡（描边色→气泡、阴影色→边框、文字色→文本、Outline→边框粗细）
- 对于 `SRT/LRC`，字幕编辑结果仅写入项目配置 `content`，不会回写 ASS 文件
...
- 波形区域使用独立容器，只有音频真正加载后才显示，不会在无音频或加载失败时占位
...
- 新增 `SRT/LRC` 字幕导入，默认使用普通说话人，编辑结果写入项目配置 `content`
- 导入 ASS 时支持样式映射与预设生成（颜色/透明度/边框粗细）
- 网页端支持预设导入/导出，桌面与网页行为对齐
- 优化手机网页布局：预览缩放、底部面板折叠/展开/拖拽、播放控件紧凑化
- 波形区域改为独立容器，仅在音频成功加载后显示，避免空占空间
